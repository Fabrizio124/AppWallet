    ...
    override fun onViewCreated(view: View, savedInstanceState: Bundle?) {
        ...
        transferButton.setOnClickListener{
           ...
           val runnable = Runnable {
               ...
               val trasferList = ApplicationDatabase.getAppDatabase(context!!)
                  .getDAO().findTransferByUserName("PlatziUser")
               trasferList.forEach { userTransferString += "\n" +it }
               Toast.makeText(context!!, userTransferString, Toast.LENGTH_LONG).show()
           }
            val handler = Handler()
